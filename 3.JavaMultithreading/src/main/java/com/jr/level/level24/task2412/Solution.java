package com.jr.level.level24.task2412;
    ...
    public static void sort(List<Stock> list) {
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                //Если ссылки на сток одинаковы значит это один обьект
                if(stock1 == stock2) return 0;
                //Получаем ссылки на имена стоков
                String name1 = (String) stock1.get("name");
                String name2 = (String) stock2.get("name");
                int res = name1.compareTo(name2);

                //Получить количество миллисекунд кратные суткам
                if (res == 0){
                    SimpleDateFormat sd = new SimpleDateFormat("yyyMMdd");
                    String date11 = sd.format(stock1.get("date"));
                    String date12 = sd.format(stock2.get("date"));
                    //Получить количество миллисекунд кратные суткам
                    res = date12.compareTo(date11);
                    if(res == 0){

                        //если change существует то профит = change
                        //иначе разнице last - open
                        double profit1 = stock1.containsKey("change")?
                                (double) stock1.get("change"):
                                (double) stock1.get("last") - (double) stock1.get("open");
                        double profit2  = stock2.containsKey("change")?
                                (double) stock2.get("change"):
                                (double) stock2.get("last") - (double) stock2.get("open");
                        res = Double.compare(profit2, profit1);
                    }
                }
                return res;
            }
